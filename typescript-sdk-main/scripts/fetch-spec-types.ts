...
async function fetchLatestSHA(): Promise<string> {
  const url =
    "https://api.github.com/repos/modelcontextprotocol/modelcontextprotocol/commits?path=schema/draft/schema.ts&per_page=1";
...
async function fetchSpecTypes(sha: string): Promise<string> {
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to fetch spec types: ${response.status} ${response.statusText}`,
    );
  }
