 ...
 */
export function createExpressResponseMock(
  options: { trackRedirectUrl?: boolean } = {},
): Response & {
  ...
  const res: Partial<Response> & { getRedirectUrl?: () => string } = {
    redirect: vi.fn(
      (urlOrStatus: string | number, maybeUrl?: string | number) => {
    ...
    set: vi.fn<Response["set"]>().mockImplementation(
      (_field: string, _value?: string | string[]) => {
        // header value is ignored in the generic mock; tests spy on set()
        return res as Response;
      },
    ),
    header: vi.fn<Response["header"]>().mockImplementation(
      (_field: string, _value?: string | string[]) => {
        return res as Response;
      },
    ),
