...
describe("Task Listing with Pagination", () => {
  let client: Awaited<
    ReturnType<typeof createInMemoryTaskEnvironment>
  >["client"];
  let server: Awaited<
    ReturnType<typeof createInMemoryTaskEnvironment>
  >["server"];
  let taskStore: Awaited<
    ReturnType<typeof createInMemoryTaskEnvironment>
  >["taskStore"];
  ...
  it("should return error code -32602 for invalid cursor", async () => {
    ...
    await expect(client.experimental.tasks.listTasks("invalid-cursor")).rejects
  ...
  it("should not include related-task metadata in list response", async () => {
    ...
    expect(result._meta?.["io.modelcontextprotocol/related-task"])
      .toBeUndefined();
  });
});
