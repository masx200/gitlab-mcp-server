...
import {
  listenOnRandomPort,
  waitForTaskStatus,
} from "@modelcontextprotocol/test-helpers";
import { z } from "zod";
...
describe("Task Lifecycle Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    mcpServer.experimental.tasks.registerToolTask(
      "long-task",
      {
        ...
        inputSchema: {
          duration: z.number().describe("Duration in milliseconds").default(
            1000,
          ),
          shouldFail: z.boolean().describe("Whether the task should fail")
            .default(false),
        },
      },
      {
              ...
              if (shouldFail) {
                ...
                await extra.taskStore.storeTaskResult(
                  ...
                  "completed",
                  {
                    content: [{
                      type: "text",
                      text: `Completed after ${duration}ms`,
                    }],
                  },
                );
              }
          ...
          (async () => {
            ...
            if (!userName) {
              const elicitationResult = await extra.sendRequest(
                ...
                } as unknown as TaskRequestOptions,
              );

              // Complete with the elicited name
              const name = elicitationResult.action === "accept" &&
  ...
  describe("Task Cancellation", () => {
    ...
    it("should reject cancellation of completed task with error code -32602", async () => {
      ...
      await expect(client.experimental.tasks.cancelTask(taskId)).rejects
        .toSatisfy((error: McpError) => {
    ...
    it("should deliver multiple queued messages in order", async () => {
      ...
      mcpServer.experimental.tasks.registerToolTask(
        "multi-request-task",
        {
          ...
          inputSchema: {
            requestCount: z.number().describe("Number of requests to send")
              .default(3),
          },
        },
        {
              ...
              try {
                await extra.taskStore.storeTaskResult(
                  ...
                  "completed",
                  {
                    content: [{
                      type: "text",
                      text: `Received responses: ${responses.join(", ")}`,
                    }],
                  },
                );
              } catch {
      ...
      expect(result.content).toEqual([{
        type: "text",
        text: "Received responses: Response 1, Response 2, Response 3",
      }]);
  ...
  describe("Input Required Flow", () => {
    it("should handle elicitation during tool execution", async () => {
      ...
      expect(elicitationRequestMeta?.[RELATED_TASK_META_KEY]).toEqual({
        taskId,
      });
  ...
  describe("Task Listing and Pagination", () => {
    it("should list tasks", async () => {
      ...
      expect(listResult.tasks.some((t) => taskIds.includes(t.taskId))).toBe(
        true,
      );
  ...
  describe("Error Handling", () => {
    it("should return error code -32602 for non-existent task in tasks/get", async () => {
      ...
      await expect(client.experimental.tasks.getTask("non-existent-task-id"))
    ...
    it("should return error code -32602 for non-existent task in tasks/cancel", async () => {
      ...
      await expect(client.experimental.tasks.cancelTask("non-existent-task-id"))
        .rejects.toSatisfy((error: McpError) => {
          expect(error).toBeInstanceOf(McpError);
          expect(error.code).toBe(ErrorCode.InvalidParams);
          expect(error.message).toContain("Task not found");
  ...
  describe("Task Cancellation with Queued Messages", () => {
    it(
      "should clear queue and deliver no messages when task is cancelled before tasks/result",
      async () => {
        ...
        mcpServer.experimental.tasks.registerToolTask(
          "cancellable-task",
          {
            ...
            inputSchema: {
              messageCount: z.number().describe("Number of messages to queue")
                .default(2),
            },
          },
          {
  ...
  describe("Continuous Message Delivery", () => {
    it(
      "should deliver messages immediately while tasks/result is blocking",
      async () => {
        ...
        mcpServer.experimental.tasks.registerToolTask(
          "streaming-task",
          {
            ...
            inputSchema: {
              messageCount: z.number().describe("Number of messages to send")
                .default(3),
              delayBetweenMessages: z.number().describe(
                ...
                try {
                  ...
                  for (let i = 0; i < messageCount; i++) {
                    ...
                    if (
                      ...
                      responses.push(
                        elicitationResult.content.response as string,
                      );
                    }
        ...
        client.setRequestHandler(ElicitRequestSchema, async (request) => {
          ...
          const match = request.params.message.match(
            /Streaming message (\d+) of (\d+)/,
          );
        ...
        const timeBetweenFirstAndSecond = receivedMessages[1]!.timestamp -
          receivedMessages[0]!.timestamp;
        const timeBetweenSecondAndThird = receivedMessages[2]!.timestamp -
          receivedMessages[1]!.timestamp;
        ...
        expect(result.content).toEqual([{
          type: "text",
          text: "Received all responses: Response 1, Response 2, Response 3",
        }]);
      ...
      15000,
    ); // Increase timeout to 15 seconds to allow for message delays
  });

  describe("Terminal Task with Queued Messages", () => {
    it(
      "should deliver queued messages followed by final result for terminal task",
      async () => {
        ...
        const receivedMessages: Array<
          { type: string; message?: string; content?: unknown }
        > = [];
  ...
  describe("callToolStream with elicitation", () => {
    it(
      "should deliver elicitation via callToolStream and complete task",
      async () => {
        const client = new Client(
          {
            name: "test-client",
        ...
        const messages: Array<
          { type: string; task?: unknown; result?: unknown; error?: unknown }
        > = [];
        ...
        const result = lastMessage.result as {
          content: Array<{ type: string; text: string }>;
        };
        expect(result.content).toEqual([{
          type: "text",
          text: "Hello, StreamUser!",
        }]);
