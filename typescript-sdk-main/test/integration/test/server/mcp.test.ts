...
import {
  McpServer,
  ResourceTemplate,
} from "../../../../packages/server/src/server/mcp.js";
import {
  type ZodMatrixEntry,
  zodTestMatrix,
} from "../../../../packages/server/test/server/__fixtures__/zodTestMatrix.js";
...
describe.each(zodTestMatrix)("$zodVersionLabel", (entry: ZodMatrixEntry) => {
  ...
  describe("tool()", () => {
        ...
        expect.arrayContaining([
          {
            type: "text",
            text: expect.stringContaining(
              "Input validation error: Invalid arguments for tool test (new api)",
            ),
          },
        ]),
      );
    ...
    test("should propagate UrlElicitationRequiredError to client callers", async () => {
      ...
      await client
        ...
        .then(() => {
          throw new Error(
            "Expected callTool to throw UrlElicitationRequiredError",
          );
        })
        .catch((error) => {
    ...
    test("should validate tool names according to SEP specification", () => {
      ...
      expect(
        warningCalls.some((call) => call.includes("Tool name contains spaces")),
      ).toBe(true);
      expect(
        warningCalls.some((call) =>
  ...
  describe("prompt()", () => {
    ...
    test("should update prompt with schema", async () => {
      ...
      expect(listResult.prompts[0]!.arguments!.map((a) => a.name).sort())
        .toEqual(["name", "value"]);
  ...
  describe("Tool title precedence", () => {
    test("should follow correct title precedence: title → annotations.title → name", async () => {
      ...
      expect(tool4!.annotations?.title).toBe(
        "Annotations Title Should Not Show",
      );
      ...
      mcpServer.tool(
        ...
        async ({ restaurant, date, partySize }) => {
          ...
          if (!available) {
            ...
            if (
              result.action === "accept" && result.content?.checkAlternatives
            ) {
            ...
            content: [
              {
                type: "text",
                text:
                  `Booked table for ${partySize} at ${restaurant} on ${date}`,
              },
            ],
          };
        },
      ...
      server.registerTool(
        ...
        async (args) => {
          return {
            content: [{
              type: "text" as const,
              text: `Value: ${args.value}, Processed: ${args.processed}`,
            }],
          };
        },
      );
      ...
      server.registerTool(
        ...
        async (args) => {
          return {
            content: [{
              type: "text" as const,
              text: `${args.name}: ${args.count} -> ${args.doubled}`,
            }],
          };
        },
      );
  ...
  describe("Tool-level task hints with automatic polling wrapper", () => {
    ...
    test("should handle task cancellation during automatic polling", async () => {
      ...
      expect(result.content).toEqual([{
        type: "text" as const,
        text: expect.stringContaining("has no result stored"),
      }]);
