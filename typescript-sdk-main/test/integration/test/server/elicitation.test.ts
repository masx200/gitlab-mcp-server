...
function testElicitationFlow(
  ...
  test(`${validatorName}: should reject invalid pattern`, async () => {
    ...
    await expect(server.elicitInput(formRequestParams)).rejects.toThrow(
  ...
  test(`${validatorName}: should default missing fields from schema defaults`, async () => {
    ...
    client.setRequestHandler(ElicitRequestSchema, (request) => {
      ...
      expect((request.params as ElicitRequestFormParams).requestedSchema)
        .toEqual(testSchemaProperties);
      return {
        action: "accept",
        content: {},
      };
