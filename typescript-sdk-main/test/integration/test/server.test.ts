...
describe("Zod v3", () => {
  ...
  test("should typecheck", () => {
    ...
    const GetWeatherRequestSchema =
      (RequestSchema as unknown as z3.ZodObject<any>).extend({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const GetForecastRequestSchema =
      (RequestSchema as unknown as z3.ZodObject<any>).extend({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const WeatherForecastNotificationSchema =
      (NotificationSchema as unknown as z3.ZodObject<any>).extend({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const WeatherRequestSchema =
      (GetWeatherRequestSchema as unknown as z3.ZodObject<any>).or(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        GetForecastRequestSchema as unknown as z3.ZodObject<any>,
      ) as AnyObjectSchema;
    const WeatherNotificationSchema =
      WeatherForecastNotificationSchema as AnyObjectSchema;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const WeatherResultSchema = (ResultSchema as unknown as z3.ZodObject<any>)
      .extend({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    type InferSchema<T> = T extends z3.ZodType<infer Output, any, any> ? Output
      : never;
...
test("should wrap unexpected validator errors during elicitInput", async () => {
  ...
  await expect(
    ...
    "MCP error -32603: Error validating elicitation response: boom - validator exploded",
  );
});
...
test("should forward notification options when using elicitation completion notifier", async () => {
  ...
  const notifier = server.createElicitationCompletionNotifier(
    "elicitation-789",
    { relatedRequestId: 42 },
  );
  await notifier();
...
test("should throw when creating notifier if client lacks URL elicitation support", async () => {
  ...
  expect(() => server.createElicitationCompletionNotifier("elicitation-123"))
    .toThrow(
    ...
    await expect(
      server.createMessage({
        messages: [
          ...
          {
            role: "user",
            content: { type: "text", text: "actually nevermind" },
          },
        ],
        maxTokens: 100,
        tools: [{ name: "test_tool", inputSchema: { type: "object" } }],
...
describe("createMcpExpressApp", () => {
  ...
  test("should parse JSON bodies", async () => {
    ...
    const response = await supertest(app).post("/test").send({ hello: "world" })
  ...
  test("should reject requests with invalid Host header by default", async () => {
    ...
    const response = await supertest(app).post("/test").set(
      "Host",
      "evil.com:3000",
    ).send({});
  ...
  test("should use custom allowedHosts when provided", async () => {
    ...
    const app = createMcpExpressApp({
      host: "0.0.0.0",
      allowedHosts: ["myapp.local", "localhost"],
    });
    app.post("/test", (_req, res) => {
      res.json({ success: true });
    });
    ...
    const allowedResponse = await supertest(app).post("/test").set(
      "Host",
      "myapp.local:3000",
    ).send({});
    ...
    const rejectedResponse = await supertest(app).post("/test").set(
      "Host",
      "evil.com:3000",
    ).send({});
...
describe("Task-based execution", () => {
  ...
  test("server without TaskStore should reject task-based requests", async () => {
    ...
    await expect(client.experimental.tasks.getTask("non-existent")).rejects
  ...
  test("should automatically attach related-task metadata to nested requests during tool execution", async () => {
    ...
    let capturedElicitRequest: z4.infer<typeof ElicitRequestSchema> | null =
      null;
          ...
          (async () => {
            ...
            const result = {
              content: [
                {
                  type: "text",
                  text: `Collected username: ${
                    elicitResult.action === "accept" && elicitResult.content
                      ? (elicitResult.content as Record<string, unknown>)
                        .username
                      : "none"
                  }`,
                },
  ...
  describe("Server calling client via elicitation", () => {
    ...
    test("should query task result from client using getTaskResult", async () => {
      ...
      // Query result
      const result = await server.experimental.tasks.getTaskResult(
        taskId,
        ElicitResultSchema,
      );
  ...
  test("should handle multiple concurrent task-based tool calls", async () => {
        ...
        async createTask({ delay, taskNum }, extra) {
          ...
          (async () => {
            ...
            const result = {
              content: [{
                type: "text",
                text: `Completed task ${taskNum || "unknown"}`,
              }],
            };
            await extra.taskStore.storeTaskResult(
              task.taskId,
    ...
    const pendingRequests = Array.from(
      ...
      (_, index) =>
        client.callTool(
          {
            name: "async-tool",
            arguments: { delay: 10 + index * 5, taskNum: index + 1 },
          },
          CallToolResultSchema,
          {
            task: { ttl: 60000 },
    ...
    for (const taskId of taskIds) {
      expect(finalTaskList.tasks).toContainEqual(
  ...
  describe("Error scenarios", () => {
    ...
    test("should throw error when client queries non-existent task from server", async () => {
      ...
      await expect(client.experimental.tasks.getTask("non-existent-task"))
        .rejects.toThrow();
    });
    ...
    test("should throw error when server queries non-existent task from client", async () => {
      ...
      await expect(server.experimental.tasks.getTask("non-existent-task"))
        .rejects.toThrow();
    });
  });
});
...
test("should respect client task capabilities", async () => {
  ...
  await expect(server.experimental.tasks.getTask(taskId)).resolves.not
    .toThrow();
  ...
  await expect(
    ...
    "Client does not support task creation for sampling/createMessage",
  );
