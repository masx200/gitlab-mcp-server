...
oauthProvider = new InMemoryOAuthClientProvider(
  ...
  (redirectUrl: URL) => {
    console.log(
      `🌐 Opening browser for OAuth redirect: ${redirectUrl.toString()}`,
    );
...
function printHelp(): void {
  ...
  console.log(
    "  payment-confirm            - Test URL elicitation via error response with payment-confirm tool",
  );
  console.log(
    "  third-party-auth           - Test tool that requires third-party OAuth credentials",
  );
...
async function elicitationLoop(): Promise<void> {
  while (true) {
    ...
    while (elicitationQueue.length > 0) {
      ...
      console.log(
        `📤 Processing queued elicitation (${elicitationQueue.length} remaining)`,
      );
 ...
 */
async function elicitationRequestHandler(
  request: ElicitRequest,
): Promise<ElicitResult> {
  // If we are processing a command, handle this elicitation immediately
  if (isProcessingCommand) {
    console.log(
      "📋 Processing elicitation immediately (during command execution)",
    );
  ...
  console.log(
    `📥 Queueing elicitation request (queue size will be: ${
      elicitationQueue.length + 1
    })`,
  );
...
async function handleElicitationRequest(
  ...
  if (mode === "url") {
    return {
      action: await handleURLElicitation(
        request.params as ElicitRequestURLParams,
      ),
    };
  } else {
...
async function handleURLElicitation(
  ...
  console.log(
    "\x1b[33mThe server is requesting you to open an external URL.\x1b[0m",
  );
  console.log(
    "\x1b[33mOnly proceed if you trust this server and understand why it needs this.\x1b[0m\n",
  );
  ...
  const completionPromise = new Promise<void>((resolve, reject) => {
    const timeout = setTimeout(
      () => {
        ...
        console.log(
          `\x1b[31m❌ Elicitation ${elicitationId} timed out waiting for completion.\x1b[0m`,
        );
  ...
  console.log(
    "   The server will send a notification once you complete the action.",
  );
...
async function waitForOAuthCallback(): Promise<string> {
  return new Promise<string>((resolve, reject) => {
    ...
    server.listen(OAUTH_CALLBACK_PORT, () => {
      console.log(
        `OAuth callback server started on http://localhost:${OAUTH_CALLBACK_PORT}`,
      );
    });
  });
}
  ...
  client.setNotificationHandler(
    ...
    (notification) => {
      ...
      if (pending) {
        ...
        pendingURLElicitations.delete(elicitationId);
        console.log(
          `\x1b[32m✅ Elicitation ${elicitationId} completed!\x1b[0m`,
        );
        pending.resolve();
      } else {
        // Shouldn't happen - discard it!
        console.warn(
          `Received completion notification for unknown elicitation: ${elicitationId}`,
        );
      }
    },
  );
