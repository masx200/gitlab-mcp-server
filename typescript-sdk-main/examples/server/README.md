...
This directory contains runnable MCP **server** examples built with
`@modelcontextprotocol/server`.

For client examples, see [`../client/README.md`](../client/README.md). For
guided docs, see [`../../docs/server.md`](../../docs/server.md).
...
- **Persistent storage mode** - state stored in a database; any node can handle
  a session.
- **Local state with message routing** - stateful nodes + pub/sub routing for a
  session.
...
Configure the transport with session management, but use an external event
store:
...
combine Streamable HTTP with pub/sub routing so one node can terminate the
client connection while another node owns the session state.
