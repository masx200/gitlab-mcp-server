...
class TaskMessageQueueWithResolvers implements TaskMessageQueue {
  ...
  async dequeue(
    taskId: string,
    _sessionId?: string,
  ): Promise<QueuedMessageWithResolver | undefined> {
    const queue = this.getQueue(taskId);
    return queue.shift();
  }
  ...
  async dequeueAll(
    taskId: string,
    _sessionId?: string,
  ): Promise<QueuedMessageWithResolver[]> {
...
class TaskResultHandler {
  ...
  private async deliverQueuedMessages(
    ...
    while (true) {
      ...
      console.log(
        `[Server] Delivering queued ${message.type} message for task ${taskId}`,
      );
      ...
      if (message.type === "request") {
        ...
        try {
          ...
          if (reqMessage.resolver) {
            reqMessage.resolver.setResult(
              response as unknown as Record<string, unknown>,
            );
          }
        } catch (error) {
          if (reqMessage.resolver) {
            reqMessage.resolver.setException(
  ...
  private async waitForUpdate(taskId: string): Promise<void> {
    ...
    await Promise.race([
      this.store.waitForUpdate(taskId),
      this.queue.waitForMessage(taskId),
    ]);
  }
...
const createServer = (): Server => {
  ...
  server.setRequestHandler(
    CallToolRequestSchema,
    async (request, extra): Promise<CallToolResult | CreateTaskResult> => {
      ...
      const taskExecution = (async () => {
        try {
          ...
          if (name === "confirm_delete") {
            ...
            const result = await taskSession.elicit(
            ...
            console.log(
              `[Server] Received elicitation response: action=${result.action}, content=${
            ...
            console.log(
              `[Server] Received sampling response: ${
                haiku.substring(0, 50)
              }...`,
            );
  ...
  server.setRequestHandler(
    GetTaskPayloadRequestSchema,
    async (request, extra): Promise<GetTaskPayloadResult> => {
      const { taskId } = request.params;
