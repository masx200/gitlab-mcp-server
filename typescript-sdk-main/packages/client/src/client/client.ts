...
function applyElicitationDefaults(
  ...
  if (
    ...
    const props = schema.properties as Record<
      string,
      JsonSchemaType & { default?: unknown }
    >;
...
export function getSupportedElicitationModes(
  ...
  const supportsFormMode = hasFormCapability ||
    (!hasFormCapability && !hasUrlCapability);
...
> {
  ...
  private _listChangedDebounceTimers: Map<
    string,
  ...
  public override setRequestHandler<T extends AnyObjectSchema>(
    ...
    if (method === "elicitation/create") {
      const wrappedHandler = async (
        ...
        const requestedSchema = params.mode === "form"
          ? (params.requestedSchema as JsonSchemaType)
          : undefined;

        if (
          params.mode === "form" && validatedResult.action === "accept" &&
          validatedResult.content && requestedSchema
        ) {
  ...
  protected assertTaskCapability(method: string): void {
    assertToolsCallTaskCapability(
      this._serverCapabilities?.tasks?.requests,
      method,
      "Server",
    );
  }
