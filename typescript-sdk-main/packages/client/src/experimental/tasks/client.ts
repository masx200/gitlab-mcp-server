...
interface ClientInternal<RequestT extends Request> {
  ...
  getToolOutputValidator(
    toolName: string,
  ): ((data: unknown) => { valid: boolean; errorMessage?: string }) | undefined;
}
...
> {
  constructor(
    private readonly _client: Client<RequestT, NotificationT, ResultT>,
  ) {}
  ...
  >(
    ...
    const optionsWithTask = {
      ...
      task: options?.task ??
        (clientInternal.isToolTask(params.name) ? {} : undefined),
    };
  ...
  async getTask(
    ...
    type ClientWithGetTask = {
      getTask(
        params: { taskId: string },
        options?: RequestOptions,
      ): Promise<GetTaskResult>;
    };
    return (this._client as unknown as ClientWithGetTask).getTask(
      { taskId },
      options,
    );
  }
   ...
   * @experimental
   */
  async getTaskResult<T extends AnyObjectSchema>(
    taskId: string,
    resultSchema?: T,
    options?: RequestOptions,
  ...
  requestStream<T extends AnyObjectSchema>(
    ...
    return (this._client as unknown as ClientWithRequestStream).requestStream(
      request,
      resultSchema,
      options,
    );
