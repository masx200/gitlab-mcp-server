...
import type {
  ...
  AnySchema,
  SchemaOutput,
} from "../util/zod-compat.js";
import { safeParse } from "../util/zod-compat.js";
import {
  getMethodLiteral,
  parseWithCompat,
} from "../util/zod-json-schema-compat.js";
...
> {
            ...
            if (this._taskMessageQueue) {
              ...
              while (
                ...
                if (
                  ...
                  const resolver = this._requestResolvers.get(
                  ...
                  if (resolver) {
                    ...
                    const messageType = queuedMessage.type === "response"
                      ? "Response"
                      : "Error";
                    this._onerror(
                      new Error(
  ...
  private _onrequest(request: JSONRPCRequest, extra?: MessageExtraInfo): void {
    ...
    const relatedTaskId = request.params?._meta?.[RELATED_TASK_META_KEY]
      ?.taskId;
    ...
    if (handler === undefined) {
      ...
      if (relatedTaskId && this._taskMessageQueue) {
        this._enqueueTaskMessage(
          ...
          this._onerror(new Error(`Failed to enqueue error response: ${error}`))
        );
      } else {
        capturedTransport
          ?.send(errorResponse)
    ...
    const fullExtra: RequestHandlerExtra<SendRequestT, SendNotificationT> = {
      ...
      sendRequest: async (r, resultSchema, options?) => {
        ...
        const effectiveTaskId = requestOptions.relatedTask?.taskId ??
          relatedTaskId;
  ...
  private _onresponse(response: JSONRPCResponse | JSONRPCErrorResponse): void {
    ...
    if (
      isJSONRPCResultResponse(response) && response.result &&
    ...
    try {
      ...
      while (true) {
        ...
        const pollInterval = task.pollInterval ??
          this._options?.defaultTaskPollInterval ?? 1000;
  ...
  request<T extends AnySchema>(
    ...
    return new Promise<SchemaOutput<T>>((resolve, reject) => {
      ...
      const cancel = (reason: unknown) => {
        ...
        const error = reason instanceof McpError
          ? reason
   ...
   */
  protected async listTasks(
    params?: { cursor?: string },
    options?: RequestOptions,
  ): Promise<SchemaOutput<typeof ListTasksResultSchema>> {
  ...
  protected async cancelTask(
    ...
    options?: RequestOptions,
  ): Promise<SchemaOutput<typeof CancelTaskResultSchema>> {
   ...
   */
  private async _enqueueTaskMessage(
    taskId: string,
  ...
  private requestTaskStore(
    ...
    return {
      ...
      storeTaskResult: async (taskId, status, result) => {
        ...
        const task = await taskStore.getTask(taskId, sessionId);
        if (task) {
          const notification: TaskStatusNotification =
            TaskStatusNotificationSchema.parse({
  ...
  for (const key in additional) {
    ...
    if (isPlainObject(baseValue) && isPlainObject(addValue)) {
      result[k] = {
        ...
        ...(addValue as Record<string, unknown>),
      } as T[typeof k];
    } else {
