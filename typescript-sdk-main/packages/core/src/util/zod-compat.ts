...
export type SchemaOutput<S> = S extends z3.ZodTypeAny ? z3.infer<S>
  : S extends z4.$ZodType ? z4.output<S>
  : never;

export type SchemaInput<S> = S extends z3.ZodTypeAny ? z3.input<S>
  : S extends z4.$ZodType ? z4.input<S>
  : never;
...
export function safeParse<S extends AnySchema>(
  schema: S,
  data: unknown,
): { success: true; data: SchemaOutput<S> } | {
  success: false;
  error: unknown;
} {
  if (isZ4Schema(schema)) {
    ...
    return result as { success: true; data: SchemaOutput<S> } | {
      success: false;
      error: unknown;
    };
  }
