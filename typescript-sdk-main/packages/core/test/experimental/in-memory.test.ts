...
import {
  ...
  InMemoryTaskStore,
} from "../../src/experimental/tasks/stores/in-memory.js";
...
describe("InMemoryTaskStore", () => {
  ...
  describe("updateTaskStatus", () => {
    ...
    it("should throw if task not found", async () => {
      await expect(store.updateTaskStatus("non-existent", "working")).rejects
    ...
    describe("status lifecycle validation", () => {
      ...
      it("should reject transition from completed to any other status", async () => {
        await store.updateTaskStatus(taskId, "completed");
        await expect(store.updateTaskStatus(taskId, "working")).rejects.toThrow(
          "Cannot update task",
        );
        await expect(store.updateTaskStatus(taskId, "input_required")).rejects
          .toThrow("Cannot update task");
        await expect(store.updateTaskStatus(taskId, "failed")).rejects.toThrow(
          "Cannot update task",
        );
        await expect(store.updateTaskStatus(taskId, "cancelled")).rejects
          .toThrow("Cannot update task");
      });
      ...
      it("should reject transition from failed to any other status", async () => {
        ...
        await expect(store.updateTaskStatus(taskId, "input_required")).rejects
          .toThrow("Cannot update task");
        await expect(store.updateTaskStatus(taskId, "completed")).rejects
          .toThrow("Cannot update task");
        await expect(store.updateTaskStatus(taskId, "cancelled")).rejects
          .toThrow("Cannot update task");
      });
  ...
  describe("storeTaskResult", () => {
    ...
    it("should throw if task not found", async () => {
      await expect(store.storeTaskResult("non-existent", "completed", {}))
    ...
    it("should reject storing result for task already in completed status", async () => {
      ...
      await expect(store.storeTaskResult(taskId, "completed", secondResult))
    ...
    it("should reject storing result for task already in failed status", async () => {
      ...
      await expect(store.storeTaskResult(taskId, "failed", secondResult))
        .rejects.toThrow("Cannot store result for task");
    });
    ...
    it("should reject storing result for cancelled task", async () => {
      ...
      await expect(store.storeTaskResult(taskId, "completed", result)).rejects
        .toThrow("Cannot store result for task");
    });
  ...
  describe("getTaskResult", () => {
    it("should throw if task not found", async () => {
      await expect(store.getTaskResult("non-existent")).rejects.toThrow(
    ...
    it("should throw if task has no result stored", async () => {
      ...
      await expect(store.getTaskResult(createdTask.taskId)).rejects.toThrow(
        `Task ${createdTask.taskId} has no result stored`,
      );
    });
  ...
  describe("listTasks", () => {
    ...
    it("should throw error for invalid cursor", async () => {
      ...
      await expect(store.listTasks("non-existent-cursor")).rejects.toThrow(
        "Invalid cursor: non-existent-cursor",
      );
    });
...
describe("InMemoryTaskMessageQueue", () => {
  ...
  describe("queue size limits", () => {
    it("should throw when maxSize is exceeded", async () => {
      ...
      await expect(queue.enqueue("task-limit", message, undefined, 2)).rejects
        .toThrow("Task message queue overflow");
    });
    ...
    it("should allow enqueue when under maxSize", async () => {
      ...
      await expect(queue.enqueue("task-ok", message, undefined, 5)).resolves
        .toBeUndefined();
    });
  });
