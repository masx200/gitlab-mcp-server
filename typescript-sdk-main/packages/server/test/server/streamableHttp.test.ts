...
import {
  type ZodMatrixEntry,
  zodTestMatrix,
} from "./__fixtures__/zodTestMatrix.js";
...
describe.each(zodTestMatrix)("$zodVersionLabel", (entry: ZodMatrixEntry) => {
  ...
  describe("StreamableHTTPServerTransport with JSON Response Mode", () => {
    ...
    it("should return JSON response for batch requests", async () => {
      ...
      const listResponse = results.find((r: { id?: RequestId }) =>
        r.id === "batch-1"
      );
      const callResponse = results.find((r: { id?: RequestId }) =>
        r.id === "batch-2"
      );
  ...
  describe("StreamableHTTPServerTransport with resumability", () => {
    ...
    it("should store and replay multiple notifications sent while client is disconnected", async () => {
      ...
      await mcpServer.server.sendLoggingMessage({
        level: "info",
        data: "Missed notification 1",
      });
      await mcpServer.server.sendLoggingMessage({
        level: "info",
        data: "Missed notification 2",
      });
      await mcpServer.server.sendLoggingMessage({
        level: "info",
        data: "Missed notification 3",
      });
  ...
  describe("StreamableHTTPServerTransport POST SSE priming events", () => {
    ...
    const createEventStore = (): EventStore => {
      const storedEvents = new Map<
        string,
        { eventId: string; message: JSONRPCMessage; streamId: string }
      >();
      return {
        async storeEvent(
          streamId: string,
    ...
    it(
      "should allow client to reconnect after standalone SSE stream is closed via extra.closeStandaloneSSEStream",
      async () => {
        const result = await createTestServer({
          sessionIdGenerator: () => randomUUID(),
          eventStore: createEventStore(),
  ...
  describe("StreamableHTTPServerTransport onsessionclosed callback", () => {
    ...
    it("should call onsessionclosed callback with correct session ID when multiple sessions exist", async () => {
      ...
      const initResponse1 = await sendPostRequest(
        url1,
        TEST_MESSAGES.initialize,
      );
      const sessionId1 = initResponse1.headers.get("mcp-session-id");

      const initResponse2 = await sendPostRequest(
        url2,
        TEST_MESSAGES.initialize,
      );
