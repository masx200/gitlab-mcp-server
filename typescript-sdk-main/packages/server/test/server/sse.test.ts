...
const createMockRequest = (
  ...
  const mockReq = {
    ...
    on: vi.fn<http.IncomingMessage["on"]>().mockImplementation(
      (event, listener) => {
...
describe.each(zodTestMatrix)("$zodVersionLabel", (entry: ZodMatrixEntry) => {
  ...
  async function createTestServerWithSse(
    ...
    const port = typeof addr === "string"
      ...
      : (addr as unknown as { port: number }).port;
  ...
  describe("SSEServerTransport", () => {
    ...
    describe("handlePostMessage method", () => {
      ...
      it("should return 400 if content-type is not application/json", async () => {
        ...
        await expect(transport.handlePostMessage(mockReq, mockRes)).resolves
          .toBe(undefined);
