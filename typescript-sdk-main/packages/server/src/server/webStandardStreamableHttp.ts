  ...
  private async handlePostRequest(
    ...
    try {
      ...
      for (const message of messages) {
        ...
        if (
          isJSONRPCRequest(message) && this._eventStore &&
  ...
  private validateProtocolVersion(req: Request): Response | undefined {
    ...
    if (
      ...
      return this.createJsonErrorResponse(
        400,
        -32000,
        `Bad Request: Unsupported protocol version: ${protocolVersion} (supported versions: ${
      ...
      if (allResponsesReady) {
        ...
        if (this._enableJsonResponse && stream.resolveJson) {
          ...
          if (responses.length === 1) {
            ...
            stream.resolveJson(
              new Response(JSON.stringify(responses), { status: 200, headers }),
            );
          }
        } else {
          // End the SSE stream
