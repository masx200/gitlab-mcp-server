...
> {
    ...
    if (method === "tools/call") {
      const wrappedHandler = async (
        ...
        if (!validatedRequest.success) {
          const errorMessage = validatedRequest.error instanceof Error
        ...
        if (params.task) {
          const taskValidationResult = safeParse(
            CreateTaskResultSchema,
            result,
          );
        ...
        if (!validationResult.success) {
          const errorMessage = validationResult.error instanceof Error
      ...
      return super.setRequestHandler(
        requestSchema,
        wrappedHandler as unknown as typeof handler,
      );
    }
  ...
  async createMessage(
    ...
    if (params.messages.length > 0) {
      ...
      const previousMessage = params.messages.length > 1
      ...
      const previousContent = previousMessage
        ? Array.isArray(previousMessage.content)
          ...
          : [previousMessage.content]
        : [];
      const hasPreviousToolUse = previousContent.some((c) =>
        c.type === "tool_use"
      );
      ...
      if (hasPreviousToolUse) {
        const toolUseIds = new Set(
          previousContent.filter((c) => c.type === "tool_use").map((c) =>
            (c as ToolUseContent).id
          ),
        );
        const toolResultIds = new Set(
          lastContent.filter((c) => c.type === "tool_result").map((c) =>
            (c as ToolResultContent).toolUseId
          ),
        );
        if (
  ...
  async elicitInput(
    ...
    switch (mode) {
      ...
      case "form": {
        ...
        const formParams: ElicitRequestFormParams = params.mode === "form"
          ? (params as ElicitRequestFormParams)
          : { ...(params as ElicitRequestFormParams), mode: "form" };
        ...
        if (
          ...
          try {
            const validator = this._jsonSchemaValidator.getValidator(
              formParams.requestedSchema as JsonSchemaType,
            );
