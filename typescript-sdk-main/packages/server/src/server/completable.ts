...
export function isCompletable(
  schema: unknown,
): schema is CompletableSchema<AnySchema> {
  return !!schema && typeof schema === "object" &&
    COMPLETABLE_SYMBOL in (schema as object);
}

/**
 * Gets the completer callback from a completable schema, if it exists.
 */
export function getCompleter<T extends AnySchema>(
  schema: T,
): CompleteCallback<T> | undefined {
  const meta =
    (schema as unknown as { [COMPLETABLE_SYMBOL]?: CompletableMeta<T> })[
      COMPLETABLE_SYMBOL
    ];
 ...
 */
export function unwrapCompletable<T extends AnySchema>(
