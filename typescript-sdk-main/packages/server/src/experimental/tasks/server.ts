...
> {
  constructor(
    private readonly _server: Server<RequestT, NotificationT, ResultT>,
  ) {}
  ...
  requestStream<T extends AnySchema>(
    ...
    return (this._server as unknown as ServerWithRequestStream).requestStream(
      request,
      resultSchema,
      options,
    );
  ...
  async getTask(
    ...
    type ServerWithGetTask = {
      getTask(
        params: { taskId: string },
        options?: RequestOptions,
      ): Promise<GetTaskResult>;
    };
    return (this._server as unknown as ServerWithGetTask).getTask(
      { taskId },
      options,
    );
  }
